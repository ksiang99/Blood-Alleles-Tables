...
for (chr_num in 1:23) {
  ...
  for (i in 1:nrow(mat_genotype)) {
    for (j in 1:ncol(mat_genotype)) {
      result <-character()
        for (k in seq_along(coord_combi_lst[[j]])) {
          if (!(NA %in% coord_combi_lst[[j]])) {
            coord <- coord_combi_lst[[j]][[k]]
            alt_ind <- alt_combi_lst[[j]][[k]]
            if (all(mat_genotype[i, unlist(coord)] == alt_ind)) {
              result <- c(result, paste(alt_phenotypes[[j]][[k]], alt_allele_names[[j]][[k]], sep = "/"))
            }
          }
        }
      if (length(result) > 0) {
        df_inferred_result[i, j] <- paste(result, collapse = " | ")
      }
      else {
        df_inferred_result[i, j] <- paste(ref_phenotypes[[j]], ref_allele_names[[j]], sep = "/")
      }
    }
    if (i %% 100 == 0) {
  ...
  print(paste0("Total execution time:", elapsed_time))
