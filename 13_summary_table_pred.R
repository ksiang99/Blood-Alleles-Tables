...
df_database_sep <- read.delim(PATH_DATA_TABLE_SEP, header = TRUE)
...
for (chr_num in 1:23) {
  ...
  for (file in split_files[[as.character(chr_num)]]) {
        ...
        print(paste0("Assigning phenotypes in Chromosome ", chr_num, " (", col, ") to summary table"))
        ...
        # OUTPUT_PATH <- paste0("./R/results/predict/chr", chr_num, "_", col, "_summary_table.tsv")
